...
CAPTURE_RECEIPT_VERSIONS = {"2", "3", "4", "5", "6", "7"}
...
def verify_console_admission(fields: dict[str, str], version: str) -> None:
    ...
    if version in {"4", "5", "6", "7"} and fields.get("recorder_console_over_limit") != "false":
...
def verify(kind: str, directory: Path) -> None:
    ...
    if kind == "millennium-dos":
        ...
        if version in {"3", "4", "5", "6", "7"} and fields.get("results_raw") == "present":
