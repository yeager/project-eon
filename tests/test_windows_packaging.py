...
class WindowsPackagingTests(unittest.TestCase):
    ...
    def test_ci_installs_the_final_artifact_and_smoke_tests_the_loader(self) -> None:
        ...
        self.assertIn("$inspectProcess.ExitCode", workflow)
        self.assertIn("$helpProcess.ExitCode", workflow)
        self.assertIn("-RedirectStandardOutput $inspectStdout", workflow)
        self.assertIn("-RedirectStandardError $helpStderr", workflow)
        self.assertNotIn("$LASTEXITCODE -ne 2", workflow)
