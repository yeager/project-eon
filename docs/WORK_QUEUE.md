...
| 5 | Millennium DOS: recover first actionable state and controls | Hash-identified `TITLE.LIB`, `GX.LIB`, video-driver and title-flow profiles; the typed post-overlay continuation covers `$d39d..$d412`, fifteen exact call returns, explicit AL/runtime-byte branches, poll cycle and terminal dispatch-call boundaries; the coordinator can enter its no-capability state only from the admitted GX boundary after a new address-bound second-`INT 91h` return; the longest table handler at `$7384..$740e` is separately recompiled through all guards, loops and thirteen opaque calls | Replay of a real input → canonical state → frame/audio checkpoint | Runtime ownership, typed forwarding, checkpoint and revocation are coded and real-media tested at their independent recovery boundary. The `$44` handler remains an offline recovery entry until `$76f1` dispatch is genuinely proven. No call return, input, handler meaning or frame is invented |
