...
| 5 | Millennium DOS: recover first actionable state and controls | Hash-identified `TITLE.LIB`, `GX.LIB`, video-driver and title-flow profiles; the typed post-overlay continuation covers `$d39d..$d412`, fifteen exact call returns, explicit AL/runtime-byte branches, poll cycle and terminal dispatch-call boundaries; an explicit `$d40a → $76f1`, index `9` → `$7384` observation can create the owned typed tenth-function session; the separate `$7521..$7596` handler covers its guard, eighteen calls and explicit runtime operands | Replay of a real input → canonical state → frame/audio checkpoint | F10 runtime ownership, typed forwarding, copy-only checkpoints and revocation are coded and real-media tested at independent recovery boundaries. The `$41` handler remains offline until its dispatcher resolution is genuinely proven. No dispatch resolution, call return, input, handler meaning or frame is invented |
