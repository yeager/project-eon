...
| Deuteros | Amiga / English | Clean system/data ADFs, load ranges, bundles, VM records, bitmaps and initial audio descriptors are validated | Starts the recovered opening VM; the boundary-gated native title/main-stage route executes owned commands, sound staging, random, palette caller continuations, buffer passes, outer input and opaque/masked sprites | Opening pixels, RGB4 palette and initial PCM are sourced from original ADF data; native opaque/masked bitplanes are verified separately but do not prove a displayed frame | Complete graphics-service effects, saved-scanline/alternate rendering, audio-consumer cadence, gameplay and saves remain unverified; typed external returns/port observations are still required |
