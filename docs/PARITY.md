...
| --- | --- | --- | --- | --- | --- |
| Millennium 2.2 | DOS / English | ZIP, original DOS files and LIB resources are hash-identified | `TITLES.EXE` key-poll hand-off reaches the hash-identified `2200AD.EXE` canvas/evidence session | `TITLE.LIB` P00; `GX.LIB` IMG00/IMG01; `2200SAVE.I` positional evidence | Full UI, simulation, mutable state, saves, named controls and native helper ABI are unrecovered |
| Millennium 2.2 | DOS / Spanish | 720 KiB FAT12 image and original files are validated through cluster chains | Starts the Spanish P00 title session | Spanish `TITLE.LIB` P00, RGB6 DAC and translation, without English substitution | Spanish batch/executable hand-off is not recovered |
| Millennium 2.2 | Amiga / English | Original ADF raw loader ranges and shared resident bytes are hash-identified | Starts a bounded raw-loader session | No frame is claimed beyond the launcher; no generated substitute is used | Transformed-stage call, decoded runtime image and game loop are unrecovered |
