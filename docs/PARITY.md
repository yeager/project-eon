...
| Deuteros | Amiga / English | Clean system/data ADFs, load ranges, bundles, VM records, bitmaps and initial audio descriptors are validated | Starts the recovered opening VM; the boundary-gated native title/main-stage route now executes owned commands, sound staging, random, palette caller continuations, two no-draw buffer passes and outer input logic | Opening pixels, RGB4 palette and initial PCM are sourced from original ADF data; native-main-stage return packets do not prove a displayed frame | Complete graphics-service effects, native sprite-bearing frames, audio-consumer cadence, gameplay and saves remain unverified; typed external returns/port observations are still required |
