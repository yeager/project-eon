...
| Millennium 2.2 | DOS / Spanish | 720 KiB FAT12 image and original files are validated through cluster chains | Preservation scanner and diagnostics only; no native adapter, launcher route, CLI launch, title session, or renderer is admitted | No presentation is admitted | This edition is explicitly out of active runtime/parity scope; static identity, FAT12 structure and hash metadata remain preserved |
| Millennium 2.2 | Amiga / English | Corrected original ADF transfers and the first-stage entry are hash-identified | Materializes the genuine first transfer and stops at its `$41000` entry | No frame is claimed beyond the launcher; no generated substitute is used | Register-save effects, vector-$10 handling, transformed continuation and game loop are unrecovered |
...
| Deuteros | Amiga / English | Clean system/data ADFs, load ranges, bundles, VM records, bitmaps and initial audio descriptors are validated | Starts the recovered opening VM; the boundary-gated native title/main-stage route executes owned commands, sound staging, random, palette caller continuations, buffer passes, outer input, opaque/masked sprites and saved scanlines | Opening pixels, RGB4 palette and initial PCM are sourced from original ADF data; native bitplanes and saved-scanline transfers are verified separately but do not prove a displayed frame | Complete graphics-service effects, alternate rendering, audio-consumer cadence, gameplay and saves remain unverified; typed external returns/port observations are still required |
