...
void apply_deuteros_amiga_alternate_renderer(
    ...
    constexpr std::size_t video_link_size = 20;
    constexpr std::size_t position_helper_size = 48;
    ...
    const auto video_link = main.subspan(main_offset(plan, 0x21768, video_link_size),
        video_link_size);
    const auto position_helper = main.subspan(
        main_offset(plan, 0x2069c, position_helper_size), position_helper_size);
    if (to_hex(sha256(video_link)) != video_link_hash
    ...
    if (trace.primary_table_selector == 1 && trace.secondary_table_selector == 0) {
        if (big16(primary_table, 0) != 0xffff
            || std::any_of(primary_table.begin() + 2, primary_table.end(), [](auto byte) { return byte != 0; })
            || std::any_of(secondary_table.begin(), secondary_table.end(), [](auto byte) { return byte != 0; })) {
