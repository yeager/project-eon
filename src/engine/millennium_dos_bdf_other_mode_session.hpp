...
#include <vector>
namespace eon {
enum class MillenniumDosBdfOtherModeState { mode_four_jump, awaiting_toggle, awaiting_segment, awaiting_source_word, returned };
...
public:
    ...
    const auto& far_effects()const{return far_effects_;} const auto& port_effects()const{return port_effects_;}
...
private:
    MillenniumDosBdfOtherModeState state_; std::uint16_t entry_di_=0,segment_=0,source_=0x07fa;std::uint8_t plane_=0,row_=0;
    std::vector<MillenniumDosBdfOtherModeFarEffect> far_effects_; std::vector<MillenniumDosBdfOtherModePortEffect> port_effects_;
};
}
