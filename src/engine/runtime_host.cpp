...
MillenniumDosSessionDriveResult RuntimeHost::drive_millennium_dos_session(const std::uint32_t step_limit){if(revoking())return {false,0,MillenniumDosSessionStopReason::inactive,{},0,"Millennium deterministic session drive rejected during revocation"};return NativeSessionController::drive_millennium_dos_session(step_limit);}
...
MillenniumDosTitleInitializationObservationResult RuntimeHost::observe_millennium_dos_title_continuation(MillenniumDosTitleContinuationObservation o){if(revoking())return {false,"Title continuation rejected during revocation"};return NativeSessionController::observe_millennium_dos_title_continuation(std::move(o));}
...
RuntimeHost::drive_deuteros_amiga_session(const std::uint32_t step_limit) {
    if(revoking())return {false,0,DeuterosAmigaSessionStopReason::inactive,0,
...
RuntimeHost::deuteros_amiga_title_program_entry() const {
    if(revoking())return std::nullopt;
    return NativeSessionController::deuteros_amiga_title_program_entry();
}
DeuterosAmigaTitleDependencyObservationResult
RuntimeHost::advance_deuteros_amiga_title_program_entry(){
    if(revoking())return {false,"Deuteros title program entry rejected during source revocation"};
    return NativeSessionController::advance_deuteros_amiga_title_program_entry();
}
...
EON_HOST_DEUTEROS_TITLE(advance_deuteros_amiga_title_tail_subroutine,(),())
EON_HOST_DEUTEROS_TITLE(observe_deuteros_amiga_title_tail_initial_service_return,(const DeuterosAmigaObservedLocalCallReturn o),(o))
...
EON_HOST_DEUTEROS_TITLE(observe_deuteros_amiga_main_stage_reentry_d0,(const DeuterosAmigaObservedMainStageReentryD0 o),(o))
EON_HOST_DEUTEROS_TITLE(observe_deuteros_amiga_main_stage_first_exec_return,(const DeuterosAmigaObservedMainStageExecReturn o),(o))
...
EON_HOST_DEUTEROS_TITLE(advance_deuteros_amiga_fade_buffers,(),())
EON_HOST_DEUTEROS_TITLE(observe_deuteros_amiga_outer_counter,(const DeuterosAmigaObservedOuterCounter o),(o))
EON_HOST_DEUTEROS_TITLE(observe_deuteros_amiga_outer_service,(const DeuterosAmigaObservedLoopRequestService o),(o))
EON_HOST_DEUTEROS_TITLE(begin_deuteros_amiga_disk_transition,(const AmigaDiskKind disk),(disk))
EON_HOST_DEUTEROS_TITLE(observe_deuteros_amiga_disk_transition_palette_return,(const DeuterosAmigaObservedDiskTransitionPaletteReturn o),(o))
EON_HOST_DEUTEROS_TITLE(observe_deuteros_amiga_disk_transition_input,(const DeuterosAmigaObservedDiskTransitionInput o),(o))
...
EON_HOST_DEUTEROS_TITLE(advance_deuteros_amiga_main_stage_scheduler_pass,(),())
EON_HOST_DEUTEROS_TITLE(advance_deuteros_amiga_main_stage_record_loop,(),())
EON_HOST_DEUTEROS_TITLE(observe_deuteros_amiga_loop_request_service,(const DeuterosAmigaObservedLoopRequestService o),(o))
EON_HOST_DEUTEROS_TITLE(observe_deuteros_amiga_main_stage_loop_graphics_return,(const DeuterosAmigaObservedMainStageExecReturn o),(o))
...
EON_HOST_DEUTEROS_TITLE(observe_deuteros_amiga_main_stage_loop_scheduler_return,(const DeuterosAmigaObservedLocalCallReturn o),(o))
...
MillenniumAmigaBootstrapRelocatorObservationResult RuntimeHost::observe_millennium_amiga_view_services(const MillenniumAmigaViewServiceRuntimeObservation o){if(revoking())return{false,"View services rejected during source revocation"};return NativeSessionController::observe_millennium_amiga_view_services(o);}
