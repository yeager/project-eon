...
ReleaseRuntimeCoordinator::observe_millennium_amiga_bootstrap_first_read_return(
    const MillenniumAmigaBootstrapRelocatorObservation observation){
    MillenniumAmigaBootstrapRelocatorObservationResult result;
    if(!millennium_amiga_relocator_||!millennium_amiga_setup_return_sequence_||millennium_amiga_first_read_return_sequence_||observation.sequence<=*millennium_amiga_setup_return_sequence_){result.error="First-read return requires the later direct Defjam setup generation";return result;}
    try{auto next=*millennium_amiga_relocator_;next.observe_first_read_return(observation.instruction_address,observation.source_or_target_address);*millennium_amiga_relocator_=std::move(next);millennium_amiga_first_read_return_sequence_=observation.sequence;result.accepted=true;}catch(const std::exception&e){result.error=std::string("First-read return rejected: ")+e.what();}return result;
}
