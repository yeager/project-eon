...
MillenniumAmigaBootstrapRelocatorObservationResult NativeSessionController::observe_millennium_amiga_first_fline_handler(const MillenniumAmigaFirstFlineHandlerObservation o){if(state_!=NativeSessionState::millennium_amiga_bootstrap)return{false,"First F-line handler requires Millennium Amiga bootstrap"};return runtime_.observe_millennium_amiga_first_fline_handler(o);}
